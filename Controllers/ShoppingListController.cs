    ...
    public class ShoppingListController : ApiController
    {
        public static List<ShoppingList> shoppingLists = new List<ShoppingList>
        {
            new ShoppingList() { Id = 0, Name = "Groceries", Items = {
                    ...
                    new Item { Id = 1, Name = "Corn", ShoppingListId = 0 },
        ...
        {
            ShoppingList result =
                shoppingLists.FirstOrDefault(s => s.Id == id); // link statement

            if (result == null)
            {
                return NotFound(); // = 404 error code
            }
            return Ok(result); // 200 Ok + the ShoppingList
        }
